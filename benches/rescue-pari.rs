    ...
    num_bigint::BigUint: From<<E::ScalarField as PrimeField>::BigInt>,
{
    ...
    let setup_circuit = Uncommitted(circuit.clone());
    let (mut pk, mut vk) = ZkPari::<E>::keygen(setup_circuit, &mut rng);
    for _ in 0..num_keygen_iterations {
        let start = ark_std::time::Instant::now();
        let _cs = ZkPari::<E>::circuit_to_keygen_cs(Uncommitted(circuit.clone())).unwrap();
        keygen_prep_time += start.elapsed();

        let setup_circuit = Uncommitted(circuit.clone());
        let start = ark_std::time::Instant::now();
        (pk, vk) = ZkPari::<E>::keygen(setup_circuit, &mut rng);
    ...
    let prover_circuit = Uncommitted(circuit.clone());
    let mut proof = ZkPari::<E>::prove(prover_circuit, &pk, &mut rng).unwrap();
    for _ in 0..num_prover_iterations {
        let start = ark_std::time::Instant::now();
        let _cs = ZkPari::<E>::circuit_to_prover_cs(Uncommitted(circuit.clone())).unwrap();
        prover_prep_time += start.elapsed();
        let prover_circuit = Uncommitted(circuit.clone());
        let start = ark_std::time::Instant::now();
        proof = ZkPari::<E>::prove(prover_circuit, &pk, &mut rng).unwrap();
