    ...
    <<E as Pairing>::G1Affine as AffineRepr>::BaseField: PrimeField,
{
    ...
    let (pk, vk) = ZkPari::<E>::keygen(Uncommitted(circuit), &mut rng);
    ...
    for _ in 0..max_chunk {
        ...
        let proof = ZkPari::prove(Uncommitted(circuit), &pk, &mut rng).unwrap();
    ...
    for &per_thread in &per_thread_sizes {
        ...
        for (proof, input) in chunk.iter().cycle().take(total_n) {
            assert!(ZkPari::<E>::verify(proof, &vk, input));
        }
            ...
            let handles: Vec<_> = (0..num_threads)
                .map(|i| {
                    s.spawn(move || {
                        ...
                        ZkPari::<E>::batch_verify(chunk, vk_ref, &mut thread_rng)
                    })
                })
                .collect();
...
fn main() {
    bench_parallel_batch_verify::<Bls12_381>();
}
